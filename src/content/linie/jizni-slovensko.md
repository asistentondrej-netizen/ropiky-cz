---
title: "Jižní Slovensko"
description: "Lehké opevnění podél maďarské hranice — po Vídeňské arbitráži (listopad 1938) zčásti ztraceno, po roce 1945 vráceno. Málokdo ví, že i zde byla betonována souvislá linie řopíků — dnes z ní zůstaly desítky objektů."
region: "Slovensko — Nitranský, Banskobystrický, Košický kraj"
delka_km: 250
pocet_objektu: 253
pocet_planovano: 525
pocet_realizovano: 253
pocet_to: 0
obdobi_stavby: "1938 (urgentně, po Anschlussu)"
stav_dnes: "Sruby a řopíky v zemědělské krajině, málo dokumentované, komunitní zájem nízký. Potenciál pro průzkum."
order: 45
bounds:
  north: 48.35
  south: 47.80
  east: 22.15
  west: 17.20
tags: ["slovensko", "juzne-slovensko", "lo-vz-37", "ropik", "nitra", "komarno"]
sources:
  - "Maršálek M.: Opevnenie južného Slovenska 1938"
  - "Archív VHÚ Bratislava"
  - "Terénní průzkumy 2010–2020"
updated: 2026-04-21
---

## Strategický význam

V **létě 1938** ČSR neočekávala jen německý útok. Maďarské vedení pod Miklósem Horthym požadovalo **obnovení Uherska v předválečných hranicích** a otevřeně hrozilo vojenským řešením. ČSR tedy betonovala **také jižní hranici Slovenska** — od **Bratislavy** přes **Komárno**, **Nové Zámky**, **Lučenec**, **Košice** po **Užhorod** (tehdy československá Podkarpatská Rus).

Stavba začala **až v květnu 1938** a do září bylo **dokončeno 70 objektů** (z plánovaných zhruba 350). Většinou řopíky LO vz. 37, několik objektů vz. 36, **žádné těžké sruby** — na jih prostě nebyl čas.

## Co se tu stalo — Vídeňská arbitráž

**2. listopadu 1938** — měsíc po Mnichovu — proběhla ve Vídni arbitráž mezi Německem, Itálií, ČSR a Maďarskem. Výsledek: **jižní pás Slovenska** (a celá Podkarpatská Rus) **byl přidělen Maďarsku**. Pás zahrnoval i **většinu nově postavených řopíků** — z 70 objektů se jich do maďarského záboru dostalo **přibližně 55**.

Maďaři objekty většinou **nepoužili, nedemontovali, jen nechali v krajině**. Po roce 1945 se Slovensko vrátilo do československých hranic, ale objekty ležely dalších 40 let v pohraničním pásmu u Maďarska — **nikdo je oficiálně neevidoval**.

Dnes jsou to **„zapomenuté pevnosti"** ČSR — známé jen místním nadšencům a několika publikacím.

## Ikonické objekty

- **Komárno** — řopík u mostu Alžběty (na slovenské straně)
- **Nové Zámky** — linie řopíků u železnice
- **Lučenec** — sruby v otevřené zemědělské krajině
- **Košice** — západně od města linie řopíků podél státní silnice

## Stav dokumentace

Jižní Slovensko je **nejméně zmapovaný úsek ŘOP**. Oficiální seznam VÚA-VHA obsahuje pouze hrubé polohy; přesné GPS souřadnice **většiny objektů dosud nejsou veřejně dostupné**. Komunitní projekty jako **„Bunkre južného Slovenska"** se je pokoušejí systematicky zaznamenat.

**Výzva pro čtenáře:** Pokud znáte lokalitu řopíku na jižním Slovensku, ozvěte se přes kontaktní formulář. **Přijímáme fotky, GPS souřadnice a svědectví.**

## Co navštívit

Vzhledem k nízké dokumentaci **nedoporučujeme samostatný výlet pouze za opevněním**. Ideální je kombinace s:

- **Komárnem** (starobylá pevnost, podunajský přístav)
- **Nitranským hradem** a muzeem
- **Košicemi** (UNESCO centrum)
- **Slovenským krasom** (NP)

Objekty pak najdete „při cestě" — a často vás překvapí, **jak moc jich v krajině skutečně je**.

## Související obsah

- [LO vz. 37 — řopík, typologie](/typologie/lo-vz-37)
- [Chronologie ŘOP 1935–1938](/clanky/chronologie-rop-1935-1938)
